document.addEventListener('DOMContentLoaded', (event) => {
  ...
  const angryBoss = document.getElementById('angry-boss');
  ...
  rightButton.addEventListener('click', (event) => {
    ...
    if (currentTextIndex === 4) {
      angryBoss.classList.remove('hidden');
      personSmall.classList.add('hidden');
    } else {
      angryBoss.classList.add('hidden');
      personSmall.classList.add('hidden');
    }
  ...
  const museumButton = window.parent.document.getElementById('museum-button');
...
function redirectToDifferentPage() {
  ...
  if (iframe) {
    iframe.src = '/Website/SEMI-PRIVATE/00_START/paruresis/paruresis.html';
  }
}
